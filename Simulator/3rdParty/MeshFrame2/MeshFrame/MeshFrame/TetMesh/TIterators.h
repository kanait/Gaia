...
public:
    using iterator_category = std::forward_iterator_tag;
    using value_type = HFPtr;
    using difference_type = std::ptrdiff_t;
    using pointer = HFPtr*;
    using reference = HFPtr&;
    ...
    T_HFIterator operator++(int) { T_HFIterator tmp(this->m_pTet, this->m_id); ++m_id; return tmp; }
...
public:
    using iterator_category = std::forward_iterator_tag;
    using value_type = TEPtr;
    using difference_type = std::ptrdiff_t;
    using pointer = TEPtr*;
    using reference = TEPtr&;
    ...
    E_TEIterator operator++(int) { E_TEIterator tmp(m_pEdge, m_iter); ++m_iter; return tmp; }
...
public:
    ...
    bool reachEnd() { return m_iter == TMeshType::VertexEdgeList(m_pV)->end(); }
...
public:
    ...
    E_FIterator(EPtr pE) : pFaces(new std::set<FPtr>) {
...
public:
    ...
    HF_VIterator operator++(int) {
        HF_VIterator tmp(this->m_pHF, this->m_pHE);
        m_pHE = TMeshType::HalfEdgeNext(m_pHE);
        if (m_pHE == TMeshType::HalfFaceHalfEdge(m_pHF)) {
            m_pHE = NULL;
        }
...
public:
    ...
    HF_HEIterator operator++(int) {
        HF_HEIterator tmp(this->m_pHF, this->m_pHE);
        m_pHE = TMeshType::HalfEdgeNext(m_pHE);
        if (m_pHE == TMeshType::HalfFaceHalfEdge(m_pHF)) {
            m_pHE = NULL;
        }
...
public:
    ...
    F_VIterator operator++(int) {
        F_VIterator tmp(m_pF, this->m_pHE);
        m_pHE = TMeshType::HalfEdgeNext(m_pHE);
        if (m_pHE == TMeshType::HalfFaceHalfEdge(m_pHF)) {
            m_pHE = NULL;
        }
...
public:
    ...
    TV_InHEIterator(TVPtr pTV) 
        : m_pTV(pTV), m_pHE(TMeshType::HalfEdgeDual(TMeshType::TVertexHalfEdge(pTV))), m_pHF(TMeshType::HalfEdgeHalfFace(m_pHE)) {}
...
protected:
    /*! Private construction function, only used to generate begin, end and tmp iterator*/
    TV_InHEIterator(TVPtr pTV, HFPtr pHF, HEPtr pHE) 
        : m_pTV(pTV), m_pHE(pHE), m_pHF(pHF) {}
...
public:
    ...
    TV_TEIterator operator++(int unused) {
        TV_TEIterator tmp(m_pTV, m_pHF, m_pHE);
        m_pHE = TMeshType::HalfEdgeDual(TMeshType::HalfEdgeNext(m_pHE));
        if (m_pHF == TMeshType::HalfEdgeHalfFace(m_pHE)) {
            m_pHE = NULL;
        }
...
protected:
    /*! Private construction function, only used to generate begin, end and tmp iterator*/
    TV_TEIterator(TVPtr pTV, HFPtr pHF, HEPtr pHE) 
        : m_pTV(pTV), m_pHE(pHE), m_pHF(pHF) {}
...
public:
    using iterator_category = std::forward_iterator_tag;
    using value_type = TVPtr;
    using difference_type = std::ptrdiff_t;
    using pointer = TVPtr*;
    using reference = TVPtr&;

    V_TVIterator(VPtr pV)
        : m_pV(pV), m_iter(TMeshType::VertexTVertexList(pV)->begin()) {}
...
protected:
    V_TVIterator(VPtr pV, typename std::vector<TVPtr>::iterator iter)
        : m_pV(pV), m_iter(iter) {}
...
protected:
    ...
    int m_ID;
		...
		struct TIterators : public TIteratorCore<typename TMeshType::DataType, typename TMeshType::TVType, typename TMeshType::VType, typename TMeshType::HEType,
